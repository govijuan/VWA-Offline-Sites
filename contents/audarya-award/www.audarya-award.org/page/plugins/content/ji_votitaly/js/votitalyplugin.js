  ...
  initialize: function(options){
	...
	build: function () {
		...
		arrOfElements.each(function (el) {
			var actual_el_id = el.getElement('span.votitaly-get-id').getText();
			var actual_id = el.id;
	...
	_update_voting_description: function (box_el, num_votes, average, out_of) {
		...
		if (this.options.show_votes || this.options.show_average) {
			string += '(' + 
							(this.options.show_votes ? num_votes +' '+(num_votes == 1 ? this.options.language.vote : this.options.language.votes) : '') +
							(this.options.show_votes && this.options.show_average ? ', ' : '') +
							(this.options.show_average ? this.options.language.average+': ' + average + ' '+this.options.language.outof : '') +
						 ')';			
		}
